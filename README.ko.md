...
| **태스크 기반 라우팅** | 분해 → 병렬 태스크/파일 수 카운트 → 단일 에이전트, /batch, 또는 팀 |
...
Step 2   라우팅          분해 → 기계적? /batch : N_parallel/N_files → 단일 OR 팀
...
| 경로 | 조건 | 소요 |
|------|------|------|
| /batch | 기계적 변환 (일괄 치환, 포맷, 반복 패턴) | 수 초 |
| 단일 에이전트 | N_parallel < 3 또는 N_files < 5 | 수 초 |
| 팀 MEDIUM | N_parallel ≥ 3 AND N_files ≥ 5 | ~3분 |
...
| 역할 | 모델 | 이유 |
|------|------|------|
| COMPLEX 계획 (Step 4 서브에이전트만) | **Opus** | 인터뷰 + Wave 분해에 심층 추론 |
| 팀 에이전트 전체 (fullstack, BE/FE, architect) | **Sonnet** | 비용 효율 + 충분한 추론 |
...
| 스킬 | 트리거 | 역할 |
|------|--------|------|
| [`/spawn-team`](.claude/skills/spawn-team/SKILL.md) | "팀 구성", "spawn team" | 핵심 오케스트레이터 — 분석 → 계획 → 구성 → 실행 |
| [`/debate`](.claude/skills/debate/SKILL.md) | "debate", "아키텍처 토론" | Codex (gpt-5.4) 적대적 검토 (단독 또는 spawn-team 내부) |
| [`/ralph`](.claude/skills/ralph/SKILL.md) | "끝날 때까지", "ralph" | PRD 기반 완료 보장 루프 |
| [`/doctor`](.claude/skills/doctor/SKILL.md) | "doctor", "환경 체크" | 환경 검증 + 설정 자동 패치 |
