...
각 에이전트: `subagent_type: "general-purpose"`, `team_name`, `name: "{domain}-{role}"`, `model`, `isolation: "worktree"` (isolated 시), `run_in_background: true`

worktree 전제: `git rev-parse --is-inside-work-tree` 실패 → shared 전환.
...
```
머지 전: git diff --name-only main | grep -vE "{pattern}" → 범위 외 → revert
